...
public class ProductDaoImpl implements ProductDao {
	...
	public boolean addProductDB(Product product) {
		...
		int report = 0;
		try {
			report = jdbcTemplate.update(sql, product.getProduct_name(), product.getCompany_name(),
					product.getProduct_price(), product.getMfd_date(), product.getExp_date(), product.getQuantity(),
					product.getPurchase_date(), product.getProduct_type(), product.getSupplier_name());
	...
	public boolean updateProduct(Product product) {
		...
		try {
			jdbcTemplate.update(sql, product.getCompany_name(), product.getProduct_price(), product.getMfd_date(),
					product.getExp_date(), product.getQuantity(), product.getPurchase_date(), product.getProduct_type(),
					product.getSupplier_name(), product.getProduct_name());
		} catch (Exception e) {
			return false;
		}
		return true;
	...
	@Override
	public List<Product> productsWithMinimumStockValue(int minStockValue) {
		String sql = "select * from product where quantity <= ?";
		List<Product> minProductList;
		try {
			minProductList = jdbcTemplate.query(sql, new ProductRowMapper(), minStockValue);
		} catch (Exception e) {
			minProductList = null;
		}
	...
	@Override
	public List<Product> productReport(String purchaseDate) {
		String sql = "select * from product where purchase_date = ?";
		List<Product> purchaseProduct;
		try {
			purchaseProduct = jdbcTemplate.query(sql, new ProductRowMapper(), purchaseDate);
		}
		catch (Exception e) {
